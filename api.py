...
class _App(FastAPI, metaclass=Singleton):
    def __init__(self):
        super().__init__(
            title=API_TITLE,
            version=API_VERSION,
...
class API:
    ...
    @staticmethod
    def _create_image_file(img: Annotated[UploadFile, File()]) -> str:  # func create image file
    ...
    async def _get_image(self, img: Annotated[UploadFile, File()]) -> Response:  # get image to predict
        # create image file from "get image"
        image_path = self._create_image_file(img)

        # predict "get image"
        predict_done = self._model.predict(image_path)
    ...
    def start_local_server(self, server_port: int | None = None) -> None:  # start local server to api
        if server_port is None:
            uvicorn.run(app=self._app, port=API_SERVER_PORT)

        elif server_port is not None:
            uvicorn.run(app=self._app, port=server_port)
